...
HOST_AND_PORT="https://rnacentral.org/sequence-search-beta"

# Submit a job
JOB_ID=$(curl -s -H "${CONTENT_TYPE}" -d "${DATABASE_AND_QUERY}" ${HOST_AND_PORT}/submit-job | jq -r '.job_id')
#echo "The job id: ${JOB_ID}"

# Job status
# Expected response: "started"
INITIAL_STATUS=$(curl -s ${HOST_AND_PORT}/job-status/$JOB_ID | jq -r '.chunks | .[] | .status')
...
FINAL_STATUS=$(curl -s ${HOST_AND_PORT}/job-status/$JOB_ID | jq -r '.chunks | .[] | .status')
#echo "The final status: ${FINAL_STATUS}"

# Facets search
# Expected response: "hitCount = 23 and textSearchError = false"
FACETS_SEARCH=$(curl -s ${HOST_AND_PORT}/job-results/$JOB_ID | jq '[.hitCount,.textSearchError]')
